        ...
        <div id="login-panel">
            ...
            <form @focusin="focusin" @focusout="focusout" @submit.prevent="submit">
                ...
                <template v-if="registerMode">
                    <label>{{ t('auth.login.email_optional') }}</label>
                    <div class="input">
                        <input
                            id="email"
                            type="email"
                ...
                <button id="login" type="submit" name="login" class="submit" :title="t('auth.login.login')">
                    <span v-if="registerMode">{{ t('auth.login.register') }}</span>
                    <span v-else>{{ t('auth.login.login') }}</span>
                </button>
            </form>
            <template v-if="allowRegister">
                <h4><span>{{ t('common.or') }}</span></h4>
                <button class="submit" @click="registerMode = !registerMode" :title="t('auth.login.register')">
                    <span v-if="registerMode">{{ t('auth.login.return') }}</span>
                    <span v-else>{{ t('auth.login.register') }}</span>
                </button>
            </template>
        </div>
    </div>
