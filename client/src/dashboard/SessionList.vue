...
<template>
    <div id="content">
        <div v-if="sessions.length === 0" id="empty">
            ...
            <div class="padding bold">没有找到你可以玩的战局哦！</div>
            <div class="padding"></div>
            <div class="bold">你是DM？</div>
            <div>在左侧的“运作”选项卡里查看你的战局！</div>
            <div class="padding">或者使用“创建”开始一个新的战局！</div>
            <div class="bold">你是一个玩家？</div>
            <div>等待你的DM给你发送一个邀请链接吧！</div>
        </div>
        ...
        <div id="details" v-if="selected">
            ...
            <div v-if="dmMode || !selected.is_locked" class="launch" @click="open(selected!)">启动！</div>
            <div v-else class="launch">房间被锁定</div>
            <div :style="{ flexGrow: 1 }"></div>
            <div class="header">最后游玩时间</div>
            ...
            <div class="leave" v-if="dmMode && exportEnabled" @click="exportCampaign">导出</div>
            <div class="leave" @click="leaveOrDelete">{{ dmMode ? "删除战局" : "退出战局" }}</div>
        </div>
    </div>
</template>
